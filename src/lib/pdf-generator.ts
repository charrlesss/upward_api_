...
class PDFReportGenerator {
    ...
    calculateScaling() {
        const contentWidth = this.columnWidths.reduce((acc, width) => acc + width, 0);
    ...
    calculateRowHeight(doc: PDFKit.PDFDocument, row: any) {
        ...
        this.keys.forEach((key, colIndex) => {
            ...
            const cellHeight = doc.heightOfString(cellValue?.toString() || '', { width: colWidth - 10 });
    ...
    drawTitleAndHeader(doc: PDFKit.PDFDocument, startY: number) {
        ...
        doc.fontSize(this.TITLE_FONT_SIZE - 3).font('Helvetica-Bold').fillColor('black');
        ...
        titleLines.forEach(line => {
            ...
            currentY += doc.heightOfString(line, {
                width: this.PAGE_WIDTH - 2 * this.MARGIN.left,
                align: 'left'
            }) + 5;
        });
        ...
        this.headers.forEach((header, colIndex) => {
            ...
            const textHeight = doc.heightOfString(header.headerName, { width: colWidth - 10 });
        ...
        this.headers.forEach((header, colIndex) => {
            ...
            doc.text(header.headerName, startX + 5, headerStartY + 5, {
                width: colWidth - 10,
                align: header.textAlign as "left" | "center" | "justify" | "right" | undefined
            });

            doc.moveTo(startX, headerStartY + maxHeaderHeight - 2)
    ...
    drawRow(doc: PDFKit.PDFDocument, row: any, rowIndex: number, startY: number) {
        ...
        this.keys.forEach((key, colIndex) => {
            ...
            if (spanInfo && colIndex > columnIndex && colIndex < columnIndex + spanLength) {
                return;
            }
            ...
            const borderDetails = this.borderedColumns.find((b: any) => b.rowIndex === rowIndex && b.columnDetails.some((c: any) => c.column === colIndex));
            if (borderDetails) {
                const { borderSides } = borderDetails;

                // Draw borders for the cell
                if (borderSides.top) {
                    doc.moveTo(startX, startY).lineTo(startX + colWidth, startY).stroke();
                }
                if (borderSides.bottom) {
                    doc.moveTo(startX, startY + this.scaledRowHeight).lineTo(startX + colWidth, startY + this.scaledRowHeight).stroke();
                }
                if (borderSides.left) {
                    doc.moveTo(startX, startY).lineTo(startX, startY + this.scaledRowHeight).stroke();
                }
                if (borderSides.right) {
                    doc.moveTo(startX + colWidth, startY).lineTo(startX + colWidth, startY + this.scaledRowHeight).stroke();
                }
            }
    ...
    generatePDF(res: Response) {
        ...
        const doc = new PDFDocument({ size: [this.PAGE_WIDTH, this.PAGE_HEIGHT], margin: 0 });
